  ...
  try {
    const response = await fetch('https://accounts.google.com/o/oauth2/token', {
      ...
      body: new URLSearchParams({
        ...
        refresh_token: token.refresh_token_google!,
      }),
      method: 'POST',
    });
    ...
    return {
      ...
      expires_in_google: getCurrentEpochTime() + tokens.expires_in,
      refresh_token_google: tokens.refresh_token ?? token.refresh_token, // Fall back to old refresh token
    };
  } catch (error) {
